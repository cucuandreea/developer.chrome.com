...
This statement holds true for both the [APE](#ape) (average percentage error) and [RMSRE_T](#rmsre) (root-mean-square percentage error with a threshold).
...
#### RMSRE  (root-mean-square percentage error with a threshold) {: #rmsre}

RMSRE_T (root-mean-square percentage error with a threshold) is another measure for noise.

##### How to interpret RMSRE_T

Lower RMSRE_T values mean better signal-to-noise ratios. 

RMSRE_T is typically higher than the average percentage error. For example, if a noise ratio that's acceptable for your use case is 20%, and RMSRE_T is 0.2, you can be confident that noise levels fall into your acceptable range.

{% Aside %}
What does it mean if RMSRE_T is above 1?
The interpretation of  RMSRE_T over 1 is similar to that of an APE over 100% LINK.

RMSRE (Root Mean Square Relative Error) is a common metric used by data scientists. 
...
2. RMSRE_T assumes the existence of minimal signal: noise/max(true-value, T). 
...
<figure>
{% Img src="image/URLGRmk9LjR39BLvmeGDZFZkz3p2/9KtDFMaEXfGejMi0cJoO.png", alt="ALT_TEXT_HERE", width="498", height="81" %}
<figcaption style="text-align:left">
...
* **RMSRE_T is more stable.** "T" is a threshold. "T" is used to give less weight in the RMSRE_T calculation to buckets that have less conversions and are therefore more sensitive to noise due to their small size. With T, the metric does not spike on buckets with few conversions. If T is equal to 5, a noise value as small as 1 on a bucket with 0 conversions will not be displayed as way over 100. Instead, it will be capped at 20, which is equivalent to 1/5, as T is equal to 5. By giving less weight to smaller buckets which are therefore more sensitive to noise, this metric is more stable, and therefore makes it easier to compare two simulations.
